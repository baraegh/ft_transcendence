    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_BACKEND_URL}/user/is_all_online`,
          { withCredentials: true }
        );
    ...
    <div>
      ...
      <div className="mainDisplay">
        ...
        <div className="friendList">
          {filteredNames
            .filter((item) => item.id !== (me?.id ?? -1)) // Filter out items with matching IDs
            .map((item) => (
              <div className="friendsSearch" key={item.id} onClick={() => {}}>
                <img className="friendsSearchImg" src={item.image} alt="" />
                <p onClick={() => {}}>
                  <Maps buttonText={item.username} id={item.id.toString()} />{" "}
                </p>
              </div>
            ))}
        </div>
